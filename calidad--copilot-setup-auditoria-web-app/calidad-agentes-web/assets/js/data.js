...
const DataManager = {
  ...
  // Inicializa datos de prueba SOLO si faltan (no destructivo)
  init() {
    ...
    ensureKey(this.STORAGE_KEYS.AUDITS, []);
    ensureKey(this.STORAGE_KEYS.METRICS, {});
    ensureKey(this.STORAGE_KEYS.WEEKLY_METRICS, {});
    ensureKey(this.STORAGE_KEYS.ACTIVITY_LOG, []);
    ensureKey(this.STORAGE_KEYS.CONNECTION_HOURS, {});

    // Default config semanas (solo si no existe)
    const existingWeekCfg = SafeStorage.getItem(this.STORAGE_KEYS.WEEK_CONFIG);
    if (!existingWeekCfg) {
      const defaultConfig = {
        '2026-0': [
    ...
    // Teams: seed/merge no destructivo
    const defaultManagementParams = {
      firstResponse: { goodMaxMin: 25, warnMaxMin: 30 },
      resolution: { goodMaxMin: 60, warnMaxMin: 90 },
      qualityMinPct: 83,
      satisfactionMinPct: 90
    };

    let teamsData = {};
    try {
      const teamsStr = SafeStorage.getItem(this.STORAGE_KEYS.TEAMS);
      teamsData = teamsStr ? JSON.parse(teamsStr) : {};
    } catch {
      teamsData = {};
    }

    // Merge base teams definitions (no borra miembros existentes)
    this.TEAMS.forEach(team => {
      const existing = teamsData[team.id];
      const seededMembers = (this.DEFAULT_AGENTS[team.id] || []).map(m => ({ ...m, team: team.id }));
      teamsData[team.id] = {
        ...(existing || {}),
        ...team,
        members: Array.isArray(existing?.members) ? existing.members : seededMembers,
        managementParams: existing?.managementParams || defaultManagementParams
      };
    });
    SafeStorage.setItem(this.STORAGE_KEYS.TEAMS, JSON.stringify(teamsData));
  },

  // Parámetros de gestión por equipo
  getTeamManagementParams(teamId) {
    const teams = this.getAllTeams();
    const params = teams?.[teamId]?.managementParams;
    return params || {
      firstResponse: { goodMaxMin: 25, warnMaxMin: 30 },
      resolution: { goodMaxMin: 60, warnMaxMin: 90 },
      qualityMinPct: 83,
  ...
  updateTeamManagementParams(teamId, partialParams) {
    const teams = this.getAllTeams();
    if (!teams[teamId]) return false;

    const current = this.getTeamManagementParams(teamId);
    const next = {
      ...current,
      ...partialParams,
      firstResponse: { ...current.firstResponse, ...(partialParams?.firstResponse || {}) },
      resolution: { ...current.resolution, ...(partialParams?.resolution || {}) }
    };

    teams[teamId].managementParams = next;
  ...
  parseTimeToSeconds(timeStr) {
    if (!timeStr || timeStr === 'Libre' || timeStr === 'VACACIONES' || timeStr === 'CAMBIO' || timeStr === 'GUARDIA' || timeStr === 'VACANTE' || timeStr === 'FALTA') {
      return 0;
    }
