...
const App = {
  ...
  importConnectionHours() {
      ...
      for (const teamId in teams) {
        ...
        if (team && team.members) {
          team.members.forEach(m => {
            if (m.role !== 'supervisor' && m.role !== 'analista' && m.role !== 'calidad') {
              if (!teamMemberNames.includes(m.name)) teamMemberNames.push(m.name);
            }
          });
          targetTeams.push(teamId);
        }
    ...
    // If es formato estado/segundos, cambiamos a modo día y acumulamos segundos por día
    ...
    for (let i = dataStartIndex; i < lines.length; i++) {
      ...
      if (isStateSecondsFormat) {
        ...
        // Asegurar que la fecha cae en la semana seleccionada o en el rango
        if (importMode === 'rango') {
      ...
      if (importMode === 'dia') {
        ...
        if (upper === 'LIBRE' || upper === '-') { status = 'libre'; hours = 'Libre'; }
          ...
          DataManager.saveAgentDailyHours(agentName, effectiveYear, monthIndex, weekIndex, selectedDate, { status, hours });
        imported++;
      } else {
        // Semana completa
        const days = {};
      ...
      Object.entries(aggregatedSeconds).forEach(([agentName, dates]) => {
        Object.entries(dates).forEach(([isoDate, totalSeconds]) => {
          ...
          if (importMode === 'rango') {
            ...
            if (wIdx !== -1) {
              targetTeams.forEach(teamId => {
                DataManager.saveAgentDailyHours(agentName, y, m, wIdx, isoDate, { status: 'worked', hours, team: teamId });
              });
              imported++;
            }
          } else {
            targetTeams.forEach(teamId => {
              DataManager.saveAgentDailyHours(agentName, effectiveYear, monthIndex, weekIndex, isoDate, { status: 'worked', hours, team: teamId });
            });
            imported++;
          }
        });
    ...
    if (importMode === 'semana') {
      Object.entries(agentData).forEach(([agentName, data]) => {
        targetTeams.forEach(teamId => {
          DataManager.saveAgentConnectionHours(agentName, effectiveYear, monthIndex, weekIndex, { ...data, team: teamId });
        });
      });
    }
